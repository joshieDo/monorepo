...
pub struct Batch<'a, E, K, V, C, I, H, U>
where
    ...
    Operation<K, V, U>: Codec,
{
    ...
    /// Operations accumulated by ancestor MerkleizedBatch chain (for flattening).
    /// Empty for top-level batches. Used for journal batch flattening and for
    /// reading operations at virtual locations during floor raise.
    parent_operations: Vec<Operation<K, V, U>>,
...
pub struct MerkleizedBatch<'a, E, K, V, C, I, H, U>
where
    E: Storage + Clock + Metrics,
    K: Key,
    V: ValueEncoding,
    ...
    Operation<K, V, U>: Codec,
{
    ...
    /// The authenticated journal's MerkleizedBatch. Contains ALL operations:
    /// parent chain ops + user mutations + floor raise ops + CommitFloor.
    journal_merkleized:
        authenticated::MerkleizedBatch<'a, H, Mmr<E, H::Digest>, Operation<K, V, U>>,
    ...
    /// All operations in the entire chain (parent operations + this batch's operations).
    /// Stored for stacking: when a child batch creates a journal batch, it prepends
    /// these as parent_operations, and reads from them for virtual locations.
    all_chain_operations: Vec<Operation<K, V, U>>,
...
impl<'a, E, K, V, C, I, H, U> Batch<'a, E, K, V, C, I, H, U>
where
    E: Storage + Clock + Metrics,
    K: Key,
    V: ValueEncoding,
...
impl<'a, E, K, V, C, I, H> Batch<'a, E, K, V, C, I, H, update::Unordered<K, V>>
where
    E: Storage + Clock + Metrics,
    K: Key,
    V: ValueEncoding,
    ...
    V::Value: Send + Sync,
{
    ...
    pub async fn merkleize(
    ...
    ) -> Result<MerkleizedBatch<'a, E, K, V, C, I, H, update::Unordered<K, V>>, Error> {
        let db = self.db;
        ...
        let mut parent_deleted_creates: Vec<(K, V::Value)> = Vec::new();
        ...
        for &loc in &locations {
            let op = read_operation_at(loc, &self.parent_operations, &[], base, &db.log).await?;
            results.push(op);
        }
        ...
        for (op, &old_loc) in results.iter().zip(&locations) {
            ...
            let base_old_loc = if let Some(parent_entry) = self.parent_overlay.get(key) {
                match parent_entry {
        ...
        for key in mutations.keys() {
            if let Some(OverlayEntry::Deleted { .. }) = self.parent_overlay.get(key) {
                if let Some(Some(value)) = mutations.get(key) {
                    parent_deleted_creates.push((key.clone(), value.clone()));
                }
            }
        }
        for (key, _) in &parent_deleted_creates {
        ...
        for (key, value) in parent_deleted_creates {
            let new_loc = Location::new_unchecked(base + ops.len() as u64);
            ops.push(Operation::Update(update::Unordered(
                key.clone(),
                value.clone(),
        ...
        if total_active_keys > 0 {
            for _ in 0..total_steps {
                ...
                loop {
                    ...
                    let op =
                        read_operation_at(op_loc, &self.parent_operations, &ops, base, &db.log)
                            .await?;
        ...
        // ============================================================
        // Phase 5: Create and merkleize the journal batch (single pass)
        // ============================================================
        // Build the full chain of operations (parent + this batch) for both
        // the journal batch and for stacking support.
        let parent_operations = self.parent_operations;
        let mut all_chain_operations = Vec::with_capacity(parent_operations.len() + ops.len());
        let mut journal_batch = db.log.new_batch();
        for op in &parent_operations {
            journal_batch.add(op.clone());
            all_chain_operations.push(op.clone());
        }
        for op in &ops {
            journal_batch.add(op.clone());
            all_chain_operations.push(op.clone());
...
impl<'a, E, K, V, C, I, H> Batch<'a, E, K, V, C, I, H, update::Ordered<K, V>>
where
    E: Storage + Clock + Metrics,
    K: Key,
    V: ValueEncoding,
    ...
    V::Value: Send + Sync,
{
    ...
    pub async fn merkleize(
        self,
        metadata: Option<V::Value>,
    ) -> Result<MerkleizedBatch<'a, E, K, V, C, I, H, update::Ordered<K, V>>, Error> {
        ...
        for &loc in &locations {
            let op: Operation<K, V, update::Ordered<K, V>> =
                read_operation_at(loc, &self.parent_operations, &[], base, &db.log).await?;
            results.push(op);
        }
        ...
        for key in deleted.iter() {
            ...
            let base_old_loc = if let Some(entry) = self.parent_overlay.get(key) {
                match entry {
                    OverlayEntry::Active { base_old_loc, .. } => *base_old_loc,
                    OverlayEntry::Deleted { .. } => {
        ...
        for (key, value) in &created {
            ...
            let prev_base_old_loc = if let Some(entry) = self.parent_overlay.get(prev_key) {
                match entry {
                    OverlayEntry::Active { base_old_loc, .. } => *base_old_loc,
                    _ => unreachable!("prev_key should be active"),
                }
            } else if let Some(entry) = overlay.get(prev_key) {
                match entry {
                    OverlayEntry::Active { base_old_loc, .. } => *base_old_loc,
                    _ => unreachable!("prev_key should be active"),
                }
...
impl<'a, E, K, V, C, I, H> MerkleizedBatch<'a, E, K, V, C, I, H, update::Ordered<K, V>>
where
    E: Storage + Clock + Metrics,
    K: Key,
    V: ValueEncoding,
...
impl<'a, E, K, V, C, I, H, U> MerkleizedBatch<'a, E, K, V, C, I, H, U>
where
    E: Storage + Clock + Metrics,
    K: Key,
    V: ValueEncoding,
    ...
    Operation<K, V, U>: Codec,
{
    ...
    ///
    /// The child batch receives a clone of this batch's overlay and a copy of all
    /// chain operations. When the child's `merkleize()` runs, it:
    /// 1. Resolves keys against the parent overlay (correctly identifying parent-created
    ///    keys as existing, not new)
    /// 2. Reads operations from parent_operations for virtual locations
    /// 3. Performs its own floor raise starting from this batch's floor location
    /// 4. Prepends all parent_operations to the journal batch (flattening)
    pub fn new_batch(&self) -> Batch<'_, E, K, V, C, I, H, U> {
        let db_journal_size = *self.db.last_commit_loc + 1;
        let total_size = db_journal_size + self.all_chain_operations.len() as u64;
        ...
        Batch {
            ...
            parent_operations: self.all_chain_operations.clone(),
...
impl<'a, E, K, V, C, I, H> MerkleizedBatch<'a, E, K, V, C, I, H, update::Unordered<K, V>>
where
    E: Storage + Clock + Metrics,
    K: Key,
    V: ValueEncoding,
...
///   ^-- base DB journal (on disk)        ^-- parent_ops (in memory)       ^-- current_ops (in memory)
/// ```
///
/// For top-level batches, parent_ops is empty, so db_journal_size == base.
...
{
    let loc_val = *loc;
    let db_journal_size = base - parent_ops.len() as u64;
    ...
    if loc_val >= base {
        ...
        // Parent batch chain's operations (in-memory).
        Ok(parent_ops[(loc_val - db_journal_size) as usize].clone())
    } else {
